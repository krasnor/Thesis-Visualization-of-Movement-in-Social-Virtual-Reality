...
public class GroupingPanel : MenuPanel
{
    ...
    /// <summary>
    ///  Disable all elements in GroupingCollection then enable or add groupings depending on the selected visualizations.
    /// </summary>
    ...
    private void CreateOrActivateGrouping(VisualizationType a_visualizationType)
    {
        ...
        if (!GroupingCollection.ContainsKey(a_visualizationType))
        {
            ...
            uiGrouping.SetContent(a_visualizationType.ToFriendlyString(), groupingCategories, true);
